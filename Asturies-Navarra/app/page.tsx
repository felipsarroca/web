...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><aside className="detail" role="dialog" aria-modal="true" aria-labelledby="activity-title" onClick={(event) => event.stopPropagation()}><button ref={closeButton} className="close" onClick={() => setSelected(null)} aria-label="Tanca la fitxa">×</button><header className="detail-head"><p className="eyebrow">{selected.block} · {selected.region.toUpperCase()}</p><h2 id="activity-title">{selected.name}</h2></header><div className="detail-facts"><span><small>Trajecte</small><b>{selected.drive}</b></span><span><small>Durada</small><b>{selected.duration}</b></span><span><small>Tipus</small><b>{selected.type}</b></span></div><section className="detail-section detail-highlight"><h3>Què hi trobareu i per què val la pena</h3><p>{selected.appeal}</p></section>{selected.practicalNotes && <section className="detail-section"><h3>Informació pràctica</h3><p>{selected.practicalNotes}</p></section>}<section className="detail-section detail-cost"><h3>Cost orientatiu</h3><p>{selected.cost}</p></section>{selected.locationPrecision === "clúster aproximat" && <p className="location-warning">La ubicació mostrada és el centre aproximat del clúster, no l’accés exacte.</p>}<div className="map-links"><a href={selected.latitude ? `https://www.openstreetmap.org/?mlat=${selected.latitude}&mlon=${selected.longitude}#map=${selected.locationPrecision === "clúster aproximat" ? 11 : 16}/${selected.latitude}/${selected.longitude}` : `https://www.openstreetmap.org/search?query=${encodeURIComponent(`${selected.name}, ${selected.region}`)}`} target="_blank" rel="noopener noreferrer">OpenStreetMap</a><a href={selected.latitude && selected.locationPrecision !== "clúster aproximat" ? `https://www.google.com/maps/search/?api=1&query=${selected.latitude},${selected.longitude}` : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${selected.name}, ${selected.region}`)}`} target="_blank" rel="noopener noreferrer">Google Maps</a></div><div className="detail-actions"><button className="primary" onClick={() => toggleSaved(selected.id)}>{saved.includes(selected.id) ? "★ Retira del pla" : "☆ Afegeix al pla"}</button><small>Comproveu horaris, reserva i accés abans de sortir.</small></div></aside></div>}

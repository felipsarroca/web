...
export default function Home() {
  ...
  const filtered = useMemo(() => activities.filter((item) => item.region === region && item.minutes <= Number(drive) && (kind === "Tots" || item.type === kind) && (!query || `${item.name} ${item.appeal} ${item.block}`.toLowerCase().includes(query.toLowerCase())) && (weather === "Qualsevol" || (weather === "Pluja" ? /museu|visita|cova|celler|aquari|centre/i.test(`${item.name} ${item.appeal}`) : /platja|cova|bosc|riu|llac|ombra/i.test(`${item.name} ${item.appeal}`)))), [activities, region, drive, kind, query, weather]);
  ...
  return <main className="app-shell">
    ...
    {tab === "Resum" && <>
      ...
      <section className="quick"><div><strong>{meta.count}</strong><span>propostes</span></div><div><strong>{meta.clusters}</strong><span>blocs de proximitat</span></div><div><strong>≤ 60′</strong><span>radi recomanat</span></div><button onClick={() => setTab("Mapa")}>Veure els indrets al mapa →</button></section>
      <section className="downloads" aria-labelledby="guides-title"><div><p className="eyebrow">GUIES DEFINITIVES 2026</p><h2 id="guides-title">Tota la informació, també fora de línia</h2><p>Horaris, aparcament, costos, combinacions i recomanacions familiars.</p></div><div className="download-actions"><a href={`${basePath}/documents/guia-asturies-2026.pdf`} download>↓ Guia d’Astúries <small>PDF · 2,4 MB</small></a><a href={`${basePath}/documents/guia-navarra-2026.pdf`} download>↓ Guia de Navarra <small>PDF · 2,0 MB</small></a></div></section>
    ...
    {tab === "Mapa" && <section className="workspace map-workspace"><div className="workspace-head"><div><p className="eyebrow">MAPA D’INDRETS · {region.toUpperCase()}</p><h1>Descobreix què teniu a prop</h1><p>Obre qualsevol punt per consultar la fitxa. Confirma sempre l’accés final i l’aparcament.</p></div><button className="primary" onClick={() => setTab("Explora")}>Veure el catàleg</button></div><ActivityMap activities={activities.filter((item) => item.region === region)} region={region} onSelect={(id) => { const activity = activities.find((item) => item.id === id); if (activity) setSelected(activity); }}/><aside className="map-note"><strong>Sobre el mapa</strong><p>Cartografia d’OpenStreetMap. La web no geocodifica ni descarrega mapes en segon pla.</p><a href="https://www.openstreetmap.org/fixthemap" target="_blank" rel="noopener noreferrer">Comunica un error del mapa</a></aside></section>}

    {tab === "Clústers" && <section className="workspace cluster-workspace"><div className="workspace-head"><div><p className="eyebrow">ESQUEMA DE PROXIMITAT DES DE {meta.base.toUpperCase()}</p><h1>Combina sense fer quilòmetres de més</h1><p>És un esquema de grups, no un mapa geogràfic. Clica un bloc per veure’n les activitats.</p></div></div><div className="cluster-layout"><div className={`cluster-map ${region === "Navarra" ? "navarra" : ""}`} aria-label="Esquema visual de clústers"><div className="rings"><i></i><i></i><i></i></div><div className="base-node"><b>{meta.base}</b><span>BASE</span></div>{clusterData[region].map((cluster, index) => <button key={cluster[0]} aria-label={`${cluster[0]}, ${cluster[1]}, ${cluster[3]} idees`} className="cluster-node" style={{ "--x": `${14 + (index * 23) % 76}%`, "--y": `${15 + (index * 37) % 72}%`, "--size": `${34 + Math.min(cluster[3], 12)}px` } as React.CSSProperties} onClick={() => openCluster(cluster[0])}><b>{cluster[0]}</b><span>{cluster[3]}</span></button>)}</div><div className="cluster-list">{clusterData[region].map((cluster) => <button key={cluster[0]} onClick={() => openCluster(cluster[0])}><span className="cluster-code">{cluster[0]}</span><span><b>{cluster[1]}</b><small>{cluster[2]} · {cluster[3]} idees</small><em>{cluster[4]}</em></span><strong>→</strong></button>)}</div></div></section>}

    {tab === "Planificador" && <section className="workspace planner"><div className="workspace-head"><div><p className="eyebrow">PLA FAMILIAR PERSISTENT</p><h1>Una àncora, un complement</h1><p>El pla queda desat en aquest dispositiu. Prioritza activitats del mateix clúster.</p></div><button className="primary" onClick={() => setTab("Explora")}>+ Afegir activitats</button></div>{savedBlocks.size > 1 && <div className="plan-warning"><b>Atenció: heu barrejat {savedBlocks.size} clústers.</b><span>Comproveu el temps total de carretera abans de considerar-ho un únic dia.</span></div>}{savedActivities.length === 0 ? <div className="empty-plan"><strong>Encara no hi ha activitats desades</strong><p>Prem l’estrella d’una proposta per afegir-la aquí.</p><button onClick={() => setTab("Explora")}>Explora activitats</button></div> : <div className="saved-list"><h2>El vostre pla provisional</h2>{savedActivities.map((activity, index) => <article key={activity.id}><span className="plan-order">{String(index + 1).padStart(2, "0")}</span><div><span>{activity.block} · {activity.drive}</span><h3>{activity.name}</h3><p>{activity.duration} · {activity.type}</p></div><button onClick={() => toggleSaved(activity.id)}>Retira</button></article>)}</div>}<aside className="specials"><h2>Regla pràctica</h2><div><article><b>1. Trieu l’àncora</b><p>La visita que justifica el desplaçament i pot ocupar mig dia.</p></article><article><b>2. Mireu el temps real</b><p>Canvieu de clúster si el navegador supera una hora.</p></article><article><b>3. Afegiu un complement</b><p>Només si queda energia i és realment a prop.</p></article></div></aside></section>}
